...
const
  ScaleFactor: Single = 32768.0; // the scalefactor scales the calculated float pcm samples to short values
...
const
  ...
  cos1_64  : single = 1.0 / (2.0 * cos(MY_PI        / 64.0));
  cos3_64  : single = 1.0 / (2.0 * cos(MY_PI * 3.0  / 64.0));
  cos5_64  : single = 1.0 / (2.0 * cos(MY_PI * 5.0  / 64.0));
  cos7_64  : single = 1.0 / (2.0 * cos(MY_PI * 7.0  / 64.0));
  cos9_64  : single = 1.0 / (2.0 * cos(MY_PI * 9.0  / 64.0));
  cos11_64 : single = 1.0 / (2.0 * cos(MY_PI * 11.0 / 64.0));
  cos13_64 : single = 1.0 / (2.0 * cos(MY_PI * 13.0 / 64.0));
  cos15_64 : single = 1.0 / (2.0 * cos(MY_PI * 15.0 / 64.0));
  cos17_64 : single = 1.0 / (2.0 * cos(MY_PI * 17.0 / 64.0));
  cos19_64 : single = 1.0 / (2.0 * cos(MY_PI * 19.0 / 64.0));
  cos21_64 : single = 1.0 / (2.0 * cos(MY_PI * 21.0 / 64.0));
  cos23_64 : single = 1.0 / (2.0 * cos(MY_PI * 23.0 / 64.0));
  cos25_64 : single = 1.0 / (2.0 * cos(MY_PI * 25.0 / 64.0));
  cos27_64 : single = 1.0 / (2.0 * cos(MY_PI * 27.0 / 64.0));
  cos29_64 : single = 1.0 / (2.0 * cos(MY_PI * 29.0 / 64.0));
  cos31_64 : single = 1.0 / (2.0 * cos(MY_PI * 31.0 / 64.0));
  cos1_32  : single = 1.0 / (2.0 * cos(MY_PI        / 32.0));
  cos3_32  : single = 1.0 / (2.0 * cos(MY_PI * 3.0  / 32.0));
  cos5_32  : single = 1.0 / (2.0 * cos(MY_PI * 5.0  / 32.0));
  cos7_32  : single = 1.0 / (2.0 * cos(MY_PI * 7.0  / 32.0));
  cos9_32  : single = 1.0 / (2.0 * cos(MY_PI * 9.0  / 32.0));
  cos11_32 : single = 1.0 / (2.0 * cos(MY_PI * 11.0 / 32.0));
  cos13_32 : single = 1.0 / (2.0 * cos(MY_PI * 13.0 / 32.0));
  cos15_32 : single = 1.0 / (2.0 * cos(MY_PI * 15.0 / 32.0));
  cos1_16  : single = 1.0 / (2.0 * cos(MY_PI        / 16.0));
  cos3_16  : single = 1.0 / (2.0 * cos(MY_PI * 3.0  / 16.0));
  cos5_16  : single = 1.0 / (2.0 * cos(MY_PI * 5.0  / 16.0));
  cos7_16  : single = 1.0 / (2.0 * cos(MY_PI * 7.0  / 16.0));
  cos1_8   : single = 1.0 / (2.0 * cos(MY_PI        / 8.0));
  cos3_8   : single = 1.0 / (2.0 * cos(MY_PI * 3.0  / 8.0));
...
function ClipInt16(Sample: Single): SmallInt;
var
  v: Integer;
begin
  v := Round(Sample);
...
//create 32 output samples
procedure TSynthesisFilter.ComputePCMSample(outbuffer: PInt16);
var
  dp: PSingle;
  pcm_sample: Single;
  pcm_sample_index, i: integer;
  pcm_sample_buffer: array[0..32-1] of single;
  vp_buff: array[0..31] of single;
  vp, vp_src: PSingle;
...
begin
  ...
  for i := 0 to 31 do begin
      move(vp_src^, vp_buff[ 0], 16*4);
      move(vp_src^, vp_buff[16], 16*4);
      pcm_sample := ((
              (vp[15] * dp[0]) +
              (vp[14] * dp[1]) +
              (vp[13] * dp[2]) +
      ...
      dp += 16;
      vp_src += 16;
      pcm_sample_buffer[pcm_sample_index] := pcm_sample;
      pcm_sample_index += 1;
  end;

  //pcm samples from both channels are interleaved in outbuffer, so skip every 2nd position
  for i := 0 to 31 do begin
      pcm_sample_clip := ClipInt16(pcm_sample_buffer[i]);
      outbuffer[i * 2] := pcm_sample_clip;
  end;
end;
