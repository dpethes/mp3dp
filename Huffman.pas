...
type
  ...
  PPHTArray = ^TPHTArray;
  TPHTArray = array[0..511] of THTArray;   //DP set to max length of huff tables
  ...
  THuffCodeTab = record
    ...
    linbits: Cardinal;               // number of linbits, max number to be stored in linbits = (1 << linbits) - 1
...
var
  ...
  // 32,33 count1-tables
  g_hufftables: array[0..HTN-1] of THuffCodeTab;

  
procedure HuffmanDecoder(h: PHuffCodeTab; var x, y, v, w: Integer; br: TBitReserve);

implementation

const
  dmask: THuffBits = THuffBits(1 shl ((sizeof(THuffBits) * 8) - 1));
...
begin
  ...
  if (h.table_number >= 32) then begin
    v := (y shr 3) and 1;
    w := (y shr 2) and 1;
    x := (y shr 1) and 1;
    y := y and 1;
  ...
  //write(format('%d:%d:%d:%d ', [x, y, v, w]));
  //writeln(h.table_number);
end;


procedure SetHuffTable(table_num: integer; xlen, ylen, linbits: Integer;
                       val: PPHTArray; treelen: Cardinal);
var
  h: PHuffCodeTab;
begin
  h := @g_hufftables[table_num];
  h.table_number := table_num;
  h.xlen := xlen;
  h.ylen := ylen;
  h.linbits := linbits;
  h.val := val;
...
begin
  SetHuffTable(0,   0, 0, 0, @ValTab0, 0);
  SetHuffTable(1,   2, 2, 0, @ValTab1, 7);
  SetHuffTable(2,   3, 3, 0, @ValTab2, 17);
  SetHuffTable(3,   3, 3, 0, @ValTab3, 17);
  SetHuffTable(4,   0, 0, 0, @ValTab4, 0);
  SetHuffTable(5,   4, 4, 0, @ValTab5, 31);
  SetHuffTable(6,   4, 4, 0, @ValTab6, 31);
  SetHuffTable(7,   6, 6, 0, @ValTab7, 71);
  SetHuffTable(8,   6, 6, 0, @ValTab8, 71);
  SetHuffTable(9,   6, 6, 0, @ValTab9, 71);

  SetHuffTable(10,  8, 8, 0, @ValTab10, 127);
  SetHuffTable(11,  8, 8, 0, @ValTab11, 127);
  SetHuffTable(12,  8, 8, 0, @ValTab12, 127);
  SetHuffTable(13,  16, 16, 0, @ValTab13, 511);
  SetHuffTable(14,  0, 0, 0, @ValTab14, 0);

  SetHuffTable(15,  16, 16,  0, @ValTab15, 511);
  SetHuffTable(16,  16, 16,  1, @ValTab16, 511);
  SetHuffTable(17,  16, 16,  2, @ValTab16, 511);
  SetHuffTable(18,  16, 16,  3, @ValTab16, 511);
  SetHuffTable(19,  16, 16,  4, @ValTab16, 511);

  SetHuffTable(20,  16, 16,  6, @ValTab16, 511);
  SetHuffTable(21,  16, 16,  8, @ValTab16, 511);
  SetHuffTable(22,  16, 16, 10, @ValTab16, 511);
  SetHuffTable(23,  16, 16, 13, @ValTab16, 511);
  SetHuffTable(24,  16, 16,  4, @ValTab24, 512);
  SetHuffTable(25,  16, 16,  5, @ValTab24, 512);
  SetHuffTable(26,  16, 16,  6, @ValTab24, 512);
  SetHuffTable(27,  16, 16,  7, @ValTab24, 512);
  SetHuffTable(28,  16, 16,  8, @ValTab24, 512);
  SetHuffTable(29,  16, 16,  9, @ValTab24, 512);
  SetHuffTable(30,  16, 16, 11, @ValTab24, 512);

  SetHuffTable(31,  16, 16, 13, @ValTab24, 512);
  SetHuffTable(32,   1, 16,  0, @ValTab32, 31);
  SetHuffTable(33,   1, 16,  0, @ValTab33, 31);
end.
